...
setwd("/home/julius_id/tpham/COVID19School/code")

source("covid19school_plotting_functions.R")
source("covid19school_functions.R")
source("covid19school_vars.R")
source("")
source("covid19school_epidemic_cluster.R")

### Packages
check.and.install.pkgs(c("data.table", 
                         "reshape2", 
                         "dplyr",
                         "truncnorm"), lib_path="/hpc/local/CentOS7/julius_id/R-libs/3.2.2/") 
...
### Paths
resultsName <- "../results/mediumIncidence_adherence80.RData"

### Scenarios
suffix <- c("", "", 
            "_vaccination",
            "_risk_based_testing","_risk_based_testing",
            "_screening_twice_weekly","_screening_weekly")
...
ncol(testing_mat)==length(day_names)
...
for(scenario in 1:length(scenarios)){
  ...
  start_time <- Sys.time()
  for(it in 1:iter){
    epidemic <- school.epidemic.cluster(seed=it, 
