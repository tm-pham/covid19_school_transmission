...
prop.symp <- function(n=1, teacher=T){
...
infec.scale <- function(n=1, mean=0.63, sd=0.125){
  return(rtruncnorm(n, a=0, mean=mean, sd=sd))
}
...
prob.trans <- function(time, infec=T, susc=F, asymp=F, vacc=F){
  ...
  prob_trans <- infectivity*ifelse(infec, infec.scale(), 1)*ifelse(vacc, susc.vacc.scale(), 1)*ifelse(susc, susc.scale(), 1)*ifelse(asymp, asymp.scale(), 1)
  return(prob_trans)
}